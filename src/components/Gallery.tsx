    ...
    <>
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item, index) => {
          ...
          return (
            <motion.button
              className={`group relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#121212] text-left ${isFeature ? 'sm:col-span-2' : ''}`}
              key={`${item.src}-${index}`}
            ...
            >
              <div className={`${isFeature ? 'aspect-[16/9]' : 'aspect-[4/3]'} overflow-hidden`}>
                {item.type === 'video' && !item.thumbnail ? (
                  <div className="grid h-full w-full place-items-center bg-[#181818] transition duration-700 group-hover:scale-105">
                    <Play className="text-[#F5A623]" size={42} />
                  </div>
                ) : (
                  <img className="h-full w-full object-cover transition duration-700 group-hover:scale-105" src={item.thumbnail ?? item.src} alt={item.title} loading="lazy" decoding="async" />
                )}
              </div>
